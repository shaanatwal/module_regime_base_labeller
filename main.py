...
from info_widget import InfoWidget # <-- IMPORT NEW WIDGET
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.info_widget = InfoWidget(self) # <-- CREATE INFO WIDGET
        self.setCentralWidget(self.welcome_screen)

        # --- Connect Signals for Interactive Info Box ---
    ...
    def handle_bar_hover(self, data_row, mouse_pos: QPoint):
        """Slot to update and show the info widget."""
        # Add a small offset so the widget doesn't sit directly under the cursor
    ...
    def open_preferences_dialog(self):
        dialog = PreferencesDialog(initial_bg_color=self.chart_widget.get_background_color(),initial_up_color=self.chart_widget.get_up_color(),initial_down_color=self.chart_widget.get_down_color(),parent=self)
        if dialog.exec() == QDialog.DialogCode.Accepted:
            bg_color, up_color, down_color = dialog.get_selected_colors()
            self.chart_widget.set_background_color(bg_color); self.chart_widget.set_up_color(up_color); self.chart_widget.set_down_color(down_color)
